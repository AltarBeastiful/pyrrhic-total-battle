...
import { Button, Card, Drawer, HelpNote, Tabs } from '@/ui/primitives';
...
function JournalList({ journal, request }: { journal: BattleJournal; request: StackRequest }) {
  ...
  return (
    <div className="space-y-2">
      ...
      <Card tone="raised" padded={false} className="overflow-x-auto p-3">
          ...
          <tbody>
            {lines.map((line) => (
              <tr key={line.n} className="border-line border-t align-top">
                <th scope="row" className="text-muted nums py-1.5 pr-2 text-right font-normal">
                  {String(line.n)}
                </th>
                <td className="py-1.5">{line.text}</td>
                <td className="text-muted nums py-1.5 pl-2 text-right">{String(line.hits)}</td>
              </tr>
            ))}
          </tbody>
        </table>
      </Card>
      <p className="text-muted nums text-xs">Total damage {amount(journal.totalDamage)}.</p>
    </div>
  );
}
...
export function JournalDrawer({ open, onOpenChange, request, journals }: JournalDrawerProps) {
  ...
  const title = tab === 'armyFirst' ? 'If you strike first' : 'If the monster strikes first';
  ...
  return (
    ...
    >
      ...
      <HelpNote className="mt-3">
        Who strikes first is a coin flip in game, which is why the summary gives you both. Enemy squads are
        named in formation order: the model gives each squad one attack per round, but it does not know which
        of them the game shows first.
      </HelpNote>
    </Drawer>
  );
}
