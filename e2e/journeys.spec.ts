...
async function journey6(page: Page, phone: boolean): Promise<void> {
  ...
  // of name: a row is **which answer it is** and nothing else (`src/ui/sections/march/picks.ts`), three
  ...
  await expect(
    ...
    trade.getByRole('row', { name: /^(Sweet spot|Most damage|Spare the stock)\b/ }),
  ).toHaveCount(rows - 1);
