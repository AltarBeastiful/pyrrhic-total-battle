...
5. **Don't duplicate information, show it where the user expects it** We want to user to be able to read easily through the information, meaning as much as possible in a continuous go without scrolling in the page. Information that are important everywhere (summary of march with damage, cost and troops; generate action) must be always visible as much as possible (even on mobile, with the good compromise)
...
6. **Copy TotalStack's forms where they are good, improve where they lack, always in our own style.** Troop selection (lowest and highest
...
16. **One page scroll.** No panes that scroll independently of the page; a supporting pane may stick. 
...
21. **Small styling problems are solved with glyphs and text colour**, not with custom components. 
...
26. **Unit details read as sentences**, not as a grid of labelled numbers. 
...
31. **Independent review of structure** (Material guidance, comparables) for any layout decision that shapes
    the whole page. 
32. **Delay what is not design**: engine stories such as best captains wait until the UI is right. 
