...
**Goal 1 — the idea the whole calculator is an improvement on** (owner, 2026-09-20: *"it should be written as
the first goal of the project: take the basic idea that high level troops correctly shielded do more damage
as a march and improve on that idea with fine-tuning"*).

The idea is one line of game knowledge: **high-tier troops, with the hired stacks correctly shielded under
them, do more damage as a march.** It follows from the one mechanic everything here is built on — the enemy
destroys **one stack per attack, always the one with the highest total HP** — so a stack's damage is decided
by its **kill position**, not by its size. A hired stack sized *under* the lowest troop rung is killed last
and strikes most; the same stack sized *over* it is killed first and strikes nothing at all. HP buys
position, never survival.

That is not a rule of thumb here, it is a measured quantity. On the owner's live account at 70 % of its
leadership, the sheltered march and the unsheltered one cost **the same 5 466 200 silver** and the sheltered
one deals **5 419 461 against 4 151 856 — 30.5 % more — while burning one chunk of hired stock less**
(experiment 118, `docs/investigations/0023-less-leadership.md`). 83 hunters carrying 2 685 730 of a march's
damage become 0 the moment the troops shrink under them.

**Everything else this app does is fine-tuning on top of that idea**, and each piece of it is a measured
gain, not a guess: which types to field and which to leave out (S-99, S-111 — the tier ladder's march is
rank 14 of 1 023 subsets on damage), how much of the stock to burn over a run of marches (the plan's bar),
how the kill order is fixed by hand (`custom`), how much of the leadership pool to fill at all (S-115), and
which of those the player should actually choose, with the facts beside the answer (S-113). The shelter is
the floor under all of it: a fine-tuning that breaks it is not a better march, and the engine applies it to
every shape it answers with (S-87, `shelterUnder`).

Goals
2. Same input coverage as TotalStack "By Source" mode, with a faster, simpler flow.
3. Deterministic, testable stacking engine that runs in the browser (Web Worker for long searches).
4. Everything persisted locally; export/import JSON; share via URL hash; no network calls at all.
5. Fully free and open source (repo LICENSE): no paywall, no tiers, no trial. Everything TotalStack puts behind
...
- Other: VIP level → army%, Dragon (13-key editor), hero (Svyatogor +50/+50 only when marching alone),
  Personal/Clan/Kingdom +25 pills, custom. VIP and the Dragon start switched **off** and TotalStack's
  "Unknown Sources" is **not** carried (owner, 2026-09-19): a figure nobody can explain is a figure
  nobody can check, and the per-key breakdown below is where a disagreement with the game is found.
...
| S-105 “Damage a hired unit” is the hired stacks' own damage per hired unit lost | done | owner, 2026-09-19, twice in one day and about the same column: ***“it says over a million but in total they do less than 1M”***, then ***“dmg per hired is still broken: it shows a damage per hired almost above total damage”***. **The defect.** `damagePerMercenary` — the bar's *Per hired*, the recap's *Damage a hired unit*, the reference table's *A mercenary*, the benchmark's `a hired` column — divided the **whole** march's (campaign's) worst opening by `mercLost`, the chunks of ten the authority pool loses for good. On a march where the troops and the monsters do most of the hitting and one or three hunters are lost, that prints nearly the damage column again: measured on his own TotalStack profile, the silver saver's worst opening is **1 000 201** and its one hired chunk was credited with all of it. **The definition.** `marchOf` already builds the enemy-first journal; it now also sums the army lines belonging to an **`authority`** stack and returns them as `hiredDamage` (one pass over entries already built — no second battle). `PlanMarch`, `PlanRepeat` and `PlanTotals` carry it, the campaign's being the repeats plus the finale, or every march of a `sequence`; the **troops-only tail adds none**, fielding no hired stack — which is why the tail no longer raises damage a hired at all, where before it handed the stock credit for 13 831 926 of damage no mercenary struck for on bear ×1. `damagePerMercenary = hiredDamage / mercLost`, `Infinity`-safe as before. **One name, one thing** (design rule 5): every rule that says *a hired* reads it — `bestFor.hired`, the trade table's column (`repeat.hiredDamage / repeat.mercLost`), the silver saver's `beatenOnBoth`, the efficient rungs the sweet spot is read off and its tie-break, the `more-mercs` tie-break, the S-99 cut's four readings, the reference table, the search's own `mostThrifty`, and the benchmark's `a hired`, `a soldier` and `a monster` (each now that group's own share of the same journal the damage column is summed from). The **undominated frontier is not** among them and says so at the definition: it is ordered on `silver`, `mercLost` and `totalDamage` — three figures, not a ratio. **The hand check** (three marches, stack by stack, against the engine to the unit): his TotalStack profile's silver saver stands Epic Monster Hunter V ×10 at kill position 10 of 13, **3** strikes at 56 280 = **168 840** for **1** chunk — 168 840 a hired against the old **1 000 201**; the 450-hunter camp's sweet spot stands Epic Monster Hunter VI ×48 at 6 of 6, **2** strikes at 776 597 = **1 553 194** for **5** chunks — **310 639** against 484 660; the monster camp's steady max stands hunters ×69 at 17 of 19 (4 strikes × 993 096) and Bear V ×4 at 19 of 19 (5 × 149 600) = **4 720 384** for **8** chunks — **590 048** against 3 222 993. **What moved.** Eleven of the sixteen benchmark armies are byte-identical outside the three ratios; on **five** the bar itself moves, because the rules that pick a stop read the column: the monster camp loses `more-mercs` (4 → 3 stops), the evening account and his 450-hunter camp lose the **silver saver** (5 → 4, 4 → 3), and the 7 000 export's and live camp's `more-mercs` land on a different rung. `bestFor.hired` moves on **one** army — his 2026-09-17 export at 12 000, silver saver → sweet spot. **Cost**: nil in work (no extra battle); the benchmark's own `planMs` over the sixteen armies reads 22 754 → 24 585 ms, inside the run-to-run noise of a loaded machine and dominated by the armies whose bar moved. **Pins**: `pnpm vitest run tests/engine` ends on **17** failures where it began on **8**; every new one is a figure that reads *a hired*, none was re-based — only the owner re-bases (the S-105 review-log entry lists all seventeen with old → new). |
| S-106 The stop rules judge on figures, so the savers come back | done | owner, 2026-09-19, the morning after S-105: *“it seems the last change made us lose some of the stops on the slider. I only get sweet spot and steady max in my usual setup.”* **The cause.** S-105 redefined *damage a hired unit* as the hired stacks' own damage over the chunks they cost — right for the column he reads — but **five rules that drop a plan read that ratio**: the silver saver's `beatenOnBoth`, the efficient rungs the sweet spot's knee is read off, the `middleOfRange` tie-break, the `more-mercs` tie-break and the S-99 cut. A ratio pair is not a domination test: both carry the same numerator, so changing *how the damage is attributed* re-orders rungs on ladders where **no march moved**. It moved five of the sixteen benchmark bars that day. **The rule now**: a march is beaten only when another has **at least its damage, at most its silver and at most its burn**, one of the three strictly better (`beatsOnFigures` — S-93's own reading, and S-94's all-in offer's); the two tie-breaks use the **campaign's** damage then its silver; `bestFor` and the printed *Per hired* keep S-105's ratio, which is what the owner reads. **Snapshot 20's bars come back on all four armies that lost something**: the monster camp 3 → **4** stops with its *more mercs* back and its sweet spot 93 298 414 → **89 196 808** at 24 chunks (snapshot 20's figure to the unit), the evening account 4 → **5** with a silver saver again, his 450-hunter camp 3 → **4** likewise, and his live camp's *more mercs* back at 37 chunks (11 029 591 → **12 086 359**). **Four bars differ from snapshot 20, every one of them on the new rule and every one a better plan**: the 7 000 export's *more mercs* stays at 47 chunks rather than 44, the campaign tie-break picking **21 363 106** over 20 079 262 for the same silver; the 4 000 case's sweet spot moves to the top rung (8 084 653 → **8 222 546** for the same 6 083 200) and its old rung becomes a **new silver saver** (5 212 861 for 3 820 600, 63 % of the silver); and the savers on the 12 000 export (2 614 000 → **2 477 300** silver) and his live account at 20 000 (4 414 200 → **3 602 400**) step left, the weaker test no longer throwing those savings away. **The S-99 cut keeps its four ratios, and the measurement is why** (disclosed, not quietly skipped): asked as figures it is never earned on the monster camp — the strongest full-set campaign there is 66 409 153 for 25 764 300 silver and the prefix-10 family reaches 97 458 367 for 36 436 800, half again the damage for 41 % more silver, so nothing of it is behind on none of the four — and the camp's sweet spot would fall **93 298 414 → 66 409 153** with its coins rising 28 200 → 31 680. The cut compares two families at different places on the frontier, which is the one comparison here a domination test cannot make; the note beside `readings` in `plan.ts` carries the numbers. **Scenario 17 is his usual setup** — Aydae 43★³3 alone, 5 200 / 2 000 / 200 dominance, guardsmen I–III with the top melee and ranged tiers out, specialists I, the monster window on tier 3, Epic Monster Hunter VI ×90, temple 0, the export's own bonuses (of its eight permanent rows only Army Modernization holds values, which is his page's *“Other: 2 of 8 sources on”*). Its steady max **reproduces his screen to the unit** — ARC1 1 313 · RD1 654 · ARC2 726 · SP2 725 · RD2 361 · RD3 203 · EMH6 32 · WE3 18 · BB3 8 · ED3 7 · SG3 6 for 2 232 600 silver, 2 976 gold, 1 080 coins, 7 d 11 h and 4 hired lost — at 3 025 937 worst against his 3 041 561, **0.51 % under**, a bonus his live account carries and this export does not. Pinned at what it measures: 2 stops, `damageFloor` **0.98**, `winsHired` true. **It still shows two stops, and that is its ladder rather than a defect**: the burn ladder is **1 · 2 · 4** and nothing else, so no rung stands inside the gap for a *more mercs*, and its thrift rung is 21 % less efficient a silver than the sweet spot (1.256 against 1.440 — 2 % of the silver saved for 15 % of the damage), which the saver's own rule refuses. Before S-105 it carried three, and the third was not a saving: the ratio rule threw the **top** rung out of the pool the knee is read off, putting the recommendation on the thriftiest march — 10 136 819 against the 11 417 051 the knee recommends now. A criterion of its own holds the rule (*a bar with a saving on it carries a silver saver*, and the saver is the **cheapest** such plan), failing on HEAD on the 12 000 export and his live account at 20 000 and passing on all seventeen after. Benchmark `tools/theorycraft/out/benchmark-2026-09-19-22-stops-on-figures.{json,md}`; nothing re-based. |
| S-107 Taking a type out re-derives the hired count up to what the shelter allows | done | owner, 2026-09-19: *“taking out one group, like SP1, doesn't compute again the mercs and I'm left with a merc stack that's below what could be added with proper shielding.”* S-104 capped every authority type of a re-size at the **selected stop's own count**, on S-80's argument that a count which can only fall keeps the rest of the plan affordable. It does — and it also makes his edit a no-op: taking a troop type out hands its leadership to the stacks that are left, the troop floor rises, and the count the stop was standing under the **old** floor is not what the new one shelters. **The cap now** is what the account can spend on every march the stop plays — `largestSustained(stock, repeats)`, the very arithmetic `planCampaign` anchors every hired type on, exported for it — or the whole authority pool for a type hired with no cap; a dominance type keeps its pool bound (S-102). The stop's own count is not a ceiling anywhere; what decides the count is the sizer and `shelterUnder`, so a **put-back** still re-derives downward exactly as it did. **Measured on his two camps**: the 450-hunter camp's sweet spot with Rider I taken out keeps the same four troop stacks (ARC2 1 625 · SP2 1 290 · RD2 660 · RD3 370) for the same **2 635 500** silver and the same 12 d 10 h of queue, its floor rises 456 624 → **696 340** HP, and the hunters go **38 → 72** — the sheltered maximum to the unit, one more standing over the floor — for **1 902 783 → 2 452 872** damage, +28.9 %, the burn 4 → 8 chunks; the cap it is given is `largestSustained(450, 3)` = **374**, never 38. His TotalStack profile with Archer I out: floor 319 260, hunters **49 → 62**, **2 016 985 → 2 163 313** (+7.3 %) for the same 2 234 900 silver and queue, burn 5 → 7, its four tier-3 monsters unmoved at 10 · 4 · 3 · 3 for 960 coins. Putting Archer I back on the first camp lowers the floor to 336 878 and answers **35** hunters for 1 987 686 under both cap rules, to the unit. The pane's line says what happened — *“… nothing else was pushed out, and your hired stacks are re-sized to what the troops shelter”* — and a type the bound puts at nothing is named in its own words (*“your … stock cannot last every march of this stop”*) rather than S-104's *“this stop spends no …”*, which is no longer the reason. `tests/engine/plan-resize.test.ts` holds it on every stop of every benchmark army — the bound is never tighter than the plan's own count, the answer fields at least what the stop did of every hired type unless the shelter or the camp's housing forbids it, and every stack is still strictly under the lowest troop stack — with the two camps' figures as a directed test. **No plan figure moves**: the re-size is outside `planCampaign`, and the benchmark run after it is byte-identical to snapshot 22 on every field but `run` and `planMs`, so no snapshot 23 was written. |
| S-108 The recap's ratios are on the worst opening, like the bar's | done | owner, 2026-09-19: *“damage/silver differs in the plan table and in the battle summary.”* He was reading two arithmetics on one screen: `simulateBattle` computes `damagePerSilver`, `damagePerGold` and `damagePerDragonCoin` from **`avgDamage`**, the midpoint of the two openings, and the recap printed them — while the plan's trade has divided the **worst opening** since S-94 (*“too risky for me to spend 3M silver on a coin flip”*). On his usual setup's steady max that is **1.3928 a silver on the recap against 1.3553 on the bar** for one march (his own screen: 1.40 against 1.36), and 2 879.30 a dragon coin against **2 801.79**. **The three midpoint fields are untouched**, and have to be: they are TotalStack's own reading of its Battle Summary (`tests/engine/summary.test.ts` pins each at `avgDamage / cost`) and what the priority search optimises when the player picks `damagePerSilver` as an objective (`src/engine/search.ts`). The **recap** reads the worst opening instead, off the two things already on its own card — `minDamage`, the figure two rows above, and the recovery bill — through `src/ui/sections/march/worst.ts`, which is also where the Details fold's damage split is summed now: `summary.damageByPool` is the midpoint too, so *Damage a hired unit* divided one arithmetic by another. The block is the **enemy-first journal**'s own army lines by pool, which makes the ratio one line of it over another *and* the bar's own `PlanRepeat.hiredDamage / mercLost` (design rule 5). Nothing in `src/engine` changed and no plan figure moves. Held two ways: `march.test.tsx` reads the rendered *Damage per silver* and *Damage per dragon coin* against `minDamage / cost` and asserts the midpoint field still differs on the same march, and `plan-criteria.test.ts` calls the recap's own functions on every stop of all seventeen benchmark armies and holds all three ratios to the bar's to the third decimal. |
| S-109 A finished run opens the March sheet, and a swipe down closes it | done | owner, 2026-09-19: *“on mobile, generate should open recap by default when finished. Also it would help to be able to close it with a gesture like sliding down if that's easy.”* **The opening is the frame's.** `useLastRun` already knew when a run landed — it writes the bar's spoken sentence and pulses its summary — so the same subscription opens the sheet, below 1200 px where the March *is* the sheet and nowhere else, a pane being on screen already. The **run count** is what is watched rather than the width, and the width is read at the moment the run lands, so a window narrowed after a run raises no sheet nobody asked for. A **re-size is not a run**: `resizeMarch` keeps the snapshot's own stamp on purpose (S-104), which is what stops a put-back made inside the sheet from re-opening it under the thumb. The bar's two signals are unchanged and still exactly right — the sentence is written only while the sheet is shut, which is now only when the frame opened nothing. **J1 on a phone is two taps** where it was three (measured, `journeys.spec.ts`): the number that changed, and Generate; the budget of 3 stands as the ceiling it always was. **The swipe** is `useDrag` from `@mantine/hooks` 9.6 (design rule 23: stock components, themed, never hand-written) on the sheet's own header, with M3's 32 × 4 drag handle drawn in it — `aria-hidden`, because the Close button and `Escape` are what a keyboard and a screen reader are given and a second control called “Close” is one too many. It closes past **96 px**, or on a downward fling (≥ 0.5 px a millisecond after 24 px), and springs back otherwise. **The sheet follows the finger on the CSS `translate` property rather than on `transform`**, and that is not a taste: Mantine's slide-up writes `transform` on that same element through React and transitions it over 250 ms, so a transform written here would be both overwritten on the next render and a quarter-second behind the hand. `translate` composes with it, is outside that transition's property list, and lets the slide-down that closes start from wherever the finger left the sheet. The spring back is a Web Animation for the same reason, guarded by `prefers-reduced-motion` and by jsdom having none. The drag surface needs `touch-action: none` or the browser claims the touch for a scroll after a few pixels and sends `pointercancel` instead of the moves; it costs the sheet's top 60 px as a place to scroll from, and nothing else. The `Drawer` is the compound one now (`Drawer.Root` / `.Content` / `.Header`), which is the only way to get a ref onto the content. **One defect fixed on the way**: Mantine paints a sticky modal header with `--mantine-color-body`, which drew a visible band across the top of the sheet in the dark scheme — it wears `--pyr-pane`, the sheet's own material, now. Held by `Shell.test.tsx` — a run opens it under 1200 px and opens nothing at 1400, a 140 px drag closes it and a 16 px pull does not (16 rather than a lifelike 40 because the other way out is a *fling*, which is a speed, and two synthetic events share a clock that cannot be flung believably) — and by `generate.spec.ts` in a real browser, where the slide-up has to **finish** before the header can be measured, its first 300 ms being spent below the window. The e2e helpers carry the rest: `generate()` puts the sheet away again so a spec that only wanted a march is left where it was, `openMarchSheet()` opens nothing when the frame already has, and `generateState()` reads the first of the two Generates that are on screen while the sheet is open — one control, one state, drawn twice by design. |
| S-110 Complete optimization leads the Battle card, and the fold's button is a button | done, default open | owner, 2026-09-19: *“also reorder complete automatization to first and default. And the button to change stacking algo should be more visible on mobile.”* **First**: `METHOD_CHOICES` leads with the plan. It is the one choice that answers the question a player brings to the app — how many marches this army can fight, how big each is and what it carries — while the other three are sizings for someone who wants to fix the order of the fall themselves. The stored ids and the order `METHODS` lists them in are untouched, so nothing migrates and no link changes meaning. **Default: not yet, and the measurement is why.** Flipping `buildSetup` to `plan` breaks the *first* run: the plan spreads the hired stock over the marches it plans, so an army that hires nothing leaves it nothing to spread, and a first-run profile holds no mercenary — the opening Generate answered *“There is no campaign to plan from this army. This method spreads the hired stock you own over the marches you set…”* instead of a march, on **26 of the built app's e2e journeys**, J1 among them. Put to the owner with the three ways out (ship it and block Generate with the reason before the press; leave the default and keep the reorder; or teach the plan to plan a mercenary-free army first, which is engine work with benchmark consequences); he chose the second **and asked to keep it open**, and `state/defaults.ts` says so at the line. The day it moves, `SHARE_SETUP_TEMPLATE` has to be **pinned at `elite`** rather than allowed to follow: what a stripped link leaves out has meant the tier ladder since v1, and a template that moved with the default would re-read every link ever written as a plan. `codec.test.ts` holds that property today and names the trap. **The button** the fold hides the other methods behind was `variant="subtle"` `size="compact-sm"` — 26 px of brass text under a bordered card, which reads as a caption *on* the card rather than as the way to the other three, and is under every touch-target floor there is (the review of 2026-09-13 made the same finding about a 34 px chip). It is `variant="default"`, `fullWidth`, 44 px, and carries the chevron that says a list opens under it: the same outline the option cards wear, so the fold reads as one block. It belongs to `ChoiceList`, so the Recovery plan's fold gained it too. No new component and no CSS. |
| S-111 The plan answers for an army that hires nothing | done | owner, 2026-09-20: *“build the troops only frontier… if we KNOW for sure it's the best play with the given configuration and we've explored all possibilities, then we've answered the complete optimization goal. Check first it's actually the case.”* **Checked, and it was not the case.** S-110 had left the method default open because `planCampaign` throws *“no feasible plan for this army”* at every account with no hired stock, and 0022 first read that as harmless — the plan's own troops-only march is `sizer([], 'elite')`, the Tier ladder's own march, so a silent fallback would have been numerically right. It is not: experiment 115 enumerated **all 1 023 subsets** of the first-run army and the Tier ladder march is **rank 14 of 1 023 on damage** (2 710 128 against 2 959 404) and rank 1 on damage a silver, with a **25-march frontier** between them; a hill-climb past the sizer's flat profile reaches 3 047 818, and a **brute force over 635 376 count vectors** on a small case says the sizer is optimal *at its own price* (nothing beats it for the silver it spends) and **75.8 %** of the best there is at any price. So the refusal was hiding an answer rather than declining an empty question. **The planner**: `planTroopsOnly` searches **tier windows ∪ greedy backward elimination ∪ the whole army**, prices every shape with the app's own sizer (`sizedShape`, which is `sizeStacks`) and returns the frontier as a bar of three stops — silver saver · sweet spot · steady max — ordered on silver, because with nothing draining `mercLost` is 0 on every row and silver is the axis the stops differ on. **Why that family and not the search's own**: experiment 116 measured five armies from ten types to a 24-type G1–G6 account; tier windows and greedy elimination each contain the **true optimum** on all three armies small enough to check every subset against, at 6 shapes priced instead of 1 023, while the family the search already had — the sizer over a **prefix** of `rankTroops` — reaches **91.6 %** of it on the first-run army and **38.9 %** of the best found on the 24-type one, the winning subset being a tier window and not a prefix of a damage-per-HP ranking. It also found that on that 24-type army the Tier ladder march is not even the best *rate*: **3.15** damage a silver against the frontier's **3.87**. **Held by `tests/engine/plan-troops-only.test.ts`** — five tests, and the contract one enumerates every subset at 4 100, 12 000 and 20 000 leadership and asserts the bar's dearest stop **is** the true optimum (`toBe`, not “close to”). **Non-regression, measured**: the gate is the exact condition the search refused — no hired type can field a unit — and every benchmark scenario holds stock (the thinnest is Bear V ×1), so nothing that answers today can reach the new path; `pnpm vitest run tests/engine` ends on the same **17** failures it began on with 467 → 476 passing, and the regenerated benchmark payload is **byte-identical on every field of all 17 scenarios but `run` and `planMs`**, so no snapshot was written. Driven on the built app at 1 400 px, a first-run account on Complete optimization draws its stops and no console error. **Adversarially reviewed before it was called done**, and the review earned its keep: the gate read *stock* where it claimed *can field*, so a bear held with no authority housing still crashed; a bar too short for a knee named one row **Sweet spot** and described another (60 of 270 swept armies have one or two stops); and exact ties survived the frontier filter, giving the reference table two rows with one key on 28 of 540. All three are fixed and each has a test; `march:` lost an unsound cast, an army whose housing fits no march throws a sentence about **housing**, and `refusalOf` no longer sends that player to the Mercenaries card. The review also re-measured the family claim on eleven armies of its own — engineers-only, a tier gap, a hand-excluded top tier — at **100 % of exhaustive truth on all eleven**. **What it exposed, and what is planned rather than done**: the bar and the table still speak of the hired stock on an army that has none — *“spending 0.0 of the hired stock”*, two dead columns, *“Fewest hired lost”* as an end label — and that, the insight table, and whether the bar can retire the Objective select are `docs/plans/total-optimization-as-the-one-answer.md` (S-112…S-114, with experiment 117's measurement: on a hired-free army the bar already answers every objective at least as well as the select, on a hired army it is 4 % short on pure damage, which is 0019 §2.1 restated). |
| S-112 The plan block speaks the army's own resources | done | The eleven hired words the S-111 review found on a bar with no stock on it, and one figure the owner replaced while it was open. **The reading**: `spendsStock(rows)` in `sections/march/picks.ts` — *does this bar trade a hired stock at all?*, asked of the **bar** and not of the stop on screen, so an army either speaks of a stock throughout or never. Off it hang: the bar's two ends (**Least silver … Most damage** where there is no stock, `barEnds`), the trade's **Hired lost** and **Per hired** columns and the row's accessible name, the curve table's **A mercenary** column, the thesis paragraph (a four-sentence version about rationing a stock, or a short one about silver and the queue), the *“Fought to the end”* line's closing clause, the Reference intro, the left-out sentence, the curve's closing note, and the bar tip's **gold** line — which was printing *“0 gold a march”* on every tip of a bar that buys none. **`bindingSentence` reads `binding.leadership`** now: the engine has always reported four bindings and the sentence weighed two, so an army sized to the last point of its pool was told *“Nothing binds yet”* with the thing that bound it on screen above. **Gold and dragon coins deliberately stay where they are** — the tip and the silver cell — because `PlanTrade` records a seventh column measured at **505 px in a 462 px pane** and cut by the owner on 2026-09-16, and this story's own gate is that the hired bar is unchanged. **The recap's hired line** (owner, 2026-09-20: *“instead of the percent of total mercs spent, replace it with the dmg per merc using a small notation: 265k, 1.23m… updated with each generate and troop left out recalculation”*): the share of the account's stock was a fact about the account, and it is **damage a hired unit** now — the hired stacks' own worst-opening damage over the hired units lost (S-105's definition, S-108's opening, summed by `worstDamageByPool` from the same journal the Details split uses). Measured on his seeded camp, the recap reads **`4 · 431.78K a hired unit`** where the bar's own *Per hired* for that stop reads **431 781**: one figure, two screens (design rule 5). `compact` gained a `decimals` argument for it, on the precedent `ratio`'s own carries — a figure compared against the last run has to be printed to where two runs differ. The row itself is drawn only where the account holds a stock, and the ratio only where the march lost some (rule 15); an **uncapped** hired type reads as stock held rather than as none. It follows a re-size as well as a Generate, because it is computed from `result.stacks` and this run's journal — held by a test that leaves a troop type out and re-reads the line. **Gates**: `pnpm vitest run src` 570 → **571**, `tests/engine` on the same 17 pre-existing failures with 476 passing, the benchmark payload byte-identical but `run` and `planMs`, `pnpm e2e` 51 passed, lint and typecheck clean. Driven in a real browser at 1 400 px on both kinds of army: the troops-only bar carries none of the eleven words and the hired bar carries every one of them. |
| S-113 The insight table: one rate the player switches, and the best of every column marked | done — the marks shipped, the rate switch was reverted | owner, 2026-09-20: *“a clear table telling you the facts summed up and letting you choose easily and switch between objective with added knowledge.”* **Measured before it was drawn** (experiment 120, `tools/theorycraft/out/120-what-the-table-should-carry.md`, over the 16 benchmark armies the baseline is registered on), because S-113 is a **layout** story before it is a data one: `docs/design.md` §7 hard-codes the column list and a seventh head measured **505 px in a 462 px pane** and was cut. Two questions decided the build. **Is a best mark worth its ink?** The bests land on **2.38 different stops on average** and one stop wins everything on only **3** armies — the ones whose bar is one or two stops long — so the marks are the table doing its job rather than a ranking with extra steps. **Which rate earns a column?** Counted as *“how often is this the only fact naming its stop”*: **Per gold is sole on 7 of 16**, *per hour of queue* on **0** (it always agrees with Silver or Per silver), *per dragon coin* exists on 2 armies and is sole on none, and **Silver, Queue and Hired lost never disagree with each other at all**. **The two rate columns were also collapsed into one with a switch over it** — Per silver · Per gold · Per hired — so three rates could share the width of two; the owner read it and reverted it the same day (*“I don't understand the selector per silver, gold, hired. Revert it.”*), so the table keeps its two fixed rate columns and no control over them. Per gold's measurement stands for whatever shape it eventually takes; what failed is a control that had to be understood before it showed anything, where every other fact on the table is visible at once. **The best figure in each column is marked**, weight first and the anchor's ink second (design rule 24: colour is never the only signal), with a `Table.Caption` saying what the mark is; nothing is marked on a one-row bar, where every row reads the same, or where two rows tie at the top. **Never on Per hired** — 0019 §2.3 measured that ratio rising while the march collapses. The marks are in each row's accessible name except the silver rate's, which the note under the name already says: experiment 120 asked the engine's own `bestFor.silver` and the column's best cell of all sixteen armies and they named the same row **16 times of 16**. Verified in a browser on the owner's seeded army before the revert — **463 px table in a 463 px wrapper, no overflow on either rate**, and Per gold's best cell (15 161 against 12 951 · 13 795 · 12 328) on a row no other column names. `docs/plans/total-optimization-as-the-one-answer.md` §3. |
| S-116 The order of death: the sheltered stacks ordered on damage a point of HP | next | owner, 2026-09-20: *“is the order of death proposed always optimal with troops, mercs and monsters? Even if we put back or left out a troop in the summary”*, against *“best damage over a campaign using my constrained resources”*. **Measured** (experiment 122, `tools/theorycraft/out/122-the-order-of-death.md`). **The arithmetic**: `health = 3 × strength` for every guardsman, specialist, mercenary and monster in the game — 56 of 65 troops, 69 of 69 mercenaries, 28 of 28 monsters, the nine exceptions being the **engineers** at 6 × — and `hitDamage` is exactly linear in the count, so a stack's **damage for each point of HP** is `(strength/health) × (100 + strengthPercent + strengthAgainst) / (100 + healthPercent)`, **independent of the count, the tier and the pool cost**. A tier-1 archer and a tier-9 kraken carry the same *base* damage per point of HP; only the bonuses tell them apart. Since the enemy kills the highest-HP stack first and the later stacks strike more, **the ideal order of death is damage-per-HP rising as total HP falls**. **The engine does not order on it**: `shelterUnder` lowers every non-leadership stack to `ceil(troopFloor / hp) − 1`, one flat ceiling per type, so where four monster types and a mercenary sit under it their relative order falls out of rounding. **On the owner's account** EMH VI reads **1.91** damage a point of HP against **1.38** for his emerald dragon and stone gargoyle, **1.23** for his battle boar and water elemental and **0.73–0.95** for his troops — and the plan kills the mercenary **fifth of nine on one blow** while four worse stacks strike twice, six inversions on one march. **What they cost: 8.3 %** — holding his troops, pools and stocks and searching only the hired counts, **8 388 740 → 9 088 494**, the whole change being **battle boar 49 → 48 and water elemental 103 → 104**; one unit off a monster drops it below two others and buys it a second blow, no authority moves, the burn is identical and the gain carries to the campaign. **With mercenaries alone the rule is already optimal** — the same search on the 2026-09-17 export finds **0 %** — so *“troops shield mercenaries”* (S-87) stands exactly as he stated it and only the ordering **among** the sheltered stacks is wrong. **A take-out makes it worse**: with the biggest troop stack left out the plan re-sizes and the mercenary lands on **zero blows**, which is the owner's own *“even if we put back or left out a troop”*. The fix keeps the shelter untouched and adds one rule under it — the higher a type's damage a point of HP, the lower its total HP — as a **local reordering** and not a search, in `shelterUnder` so every shape and the re-size (S-107) get it. **The benchmark plan is step 0 of the story**: only **2 of the 16** scenarios hold a dominance pool at all (100 and 200), so his 2026-09-20 account — 1 200 dominance, four monster types, 232 monster units — is **added and its baseline registered before the change**, or the bench cannot see the thing being fixed. A reordering that raises damage at identical silver and burn is a **win** the baseline passes; one that raises silver or burn is a **trade** that fails until the owner registers it. §D's hill-climb is the ceiling to score against (8.3 % / 0 %), `planMs` is the cost to watch, and the zero-blow take-out becomes a **regression test** rather than a benchmark row. `docs/plans/the-order-of-death.md`. |
| S-117 A March edit answers with the best march, not the biggest | done — all three changes | owner, 2026-09-20: *“I'm not that sure any more that when removing or adding a troop — left out / put back from the battle summary — we should not compute again the best possible outcome, checking if less leadership buys us something.”* **Measured** (experiment 124, `tools/theorycraft/out/124-what-the-edit-answers-with.md`, on four armies including investigation 0024's dominance account). A press runs `resizeMarchOver`: the sizer's three methods and the ten `LADDER_GROWTHS` over the types that are in, at the **full** pool, best by `beats` — **fewest unfielded, then highest damage**, silver and queue firing only on an exact draw. Three findings. **(A) The stop is not a candidate of its own re-size**, and the family is a strict subset of what `planCampaign` searched to build it, so a press can answer with a worse march. Of **14 stops re-sized over their own troop set** — the press that changes no type at all, a put-back that cannot be fielded — **3 come back with less damage for no less silver**: his live account's steady max **7 354 938 → 6 276 619 (85.3 %) for 101 % of the silver**, and 0024's account **5 763 382 → 5 143 823 for 2 498 200 against 2 449 200**, which is 0024 §5 reproduced to the unit from an independent rebuild. Over 33 real edits the churn does buy damage — the untouched march wins outright on 1 — but **18 of 33 cost more silver than the stop and 21 take longer**, because §E hands the sizer a hired budget that is not the stop's: the export's silver saver caps the hunter at **118 where the stop fields 20 (5.9×)** and each of 0024's four monster types reads the whole 800 dominance (**5–6.3×**, the pool over-subscribed four times). **(B) Less leadership does buy something after an edit** — which 119 could not find on a stop and is why S-115 was retired. 33 edits × 11 fills = **363 marches, 14 dominations** (at least the damage, no more silver, no more burn): the export at **96 % of the pool** gives **+2.2 % damage for −4 % silver at identical burn**, 0024's sweet spot at 90 % **+2.2 % for −9.6 %**, and its steady max **the same damage at 90 % as at 100 %** — the last tenth of the leadership buys nothing and costs 207 600 silver and 41 070 s of queue. **(C) Re-ranking alone is dead**: the three sizer methods answer with the identical march on all 33 edits and **0 of 33** runners-up score better under `CAMPAIGN.putBack`; the ladder's own next step down is −10.6 % damage; **every one of the 14 dominations is the sizer at a smaller pool**, a candidate that does not exist today. **Cost: 0.2–0.28 ms a re-size, ≈ 3 ms for a six-fill dial**, off the main thread. **The build**: the stop's own counts with the edit applied become a candidate and **`beats` is left alone** — a first draft had that candidate win ties outright, which is wrong (it would refuse the export's 196.8 %-damage answer) and unnecessary, because in all four cases where today's answer is worse than the march it replaced the untouched march has **more damage** (7 096 072 · 7 354 938 · 5 763 382 · 4 546 337 against 6 276 619 · 6 276 619 · 5 143 823 · 3 883 970), so damage-first picks it the moment it is on the table; the shape loop runs at `[100, 98, 96, 94, 92, 90]` and a lower fill is taken **only when it dominates**; and the 26 fills that merely pass his put-back rate are held back for him, disclosed in the line `resizeWords` already writes. **Blast radius, checked**: `resizeMarchOver` is called from `src/worker/jobs.ts:45` and from nowhere else in `src/`, and only `tests/engine/plan-resize.test.ts` (24 tests, 36 s, green) reaches it — `planCampaign` never does, so **no benchmark baseline can move**; what moves is that file's pinned counts (38 → 72, 49 → 62), each to be re-stated with the fill and the three figures that made it dominate. **Built 2026-09-21** (`caae039`, `35c440f`). **Step 1** — the stop's counts become a candidate: the three no-op presses that lost damage go to **0** and a fourth (the live all-in, 90.6 %) comes back to 100 %; the one dominated edit goes **3 883 970 → 4 546 337, +17.1 %**; the edits where churn genuinely buys damage are untouched at 169.9 % and 166.6 %. **Steps 2–3** — the dial at `CAMPAIGN.editFills` = [98, 96, 94, 92, 90], a fill taken **only when it dominates**: **9 of 102** edits on the benchmark corpus and **5 of 33** on the four armies, the export at 96 % giving **+2.2 % damage for −4 % silver at identical burn** and 0024's sweet spot at 90 % **+2.2 % for −9.6 %**. Cost **+0.83–1.18 ms** a press (0.18–0.28 → 1.0–1.5), off the main thread. **Two regression tests, both written to prove they are not vacuous**: the press that changes no type never lowers the damage — and the same test counts the stops where the pre-S-117 family falls short, failing at zero, which is **13 of 53**, worst **−22.9 %** (the 900-dominance camp answering 19 882 698 where its stop deals 25 783 940) — and every dialled answer beats the full-pool one on damage, silver and burn. **The plan was wrong twice and it was caught before it shipped**: its §1 had the untouched march win ties outright, which would have refused the export's 196.8 %-damage answer (adding the candidate is the whole fix, since the untouched march always has more damage where the old answer was worse), and its §3 expected `plan-resize`'s pins 38 → 72 and 49 → 62 to move — **neither did**, because the dial only takes a dominating march and nothing dominated on those two camps. **Gate**: `tests/engine` **17 failed | 478 passed** against the baseline's 17 | 476, the same 17 failures and the same three files, the two new tests being the difference; `plan-resize` 26/26; `pnpm vitest run src` 588/588 over 68 files; `pnpm e2e` 53 passed, 7 skipped; the regenerated benchmark payload moves **18 leaves and every one is `run` or `planMs`**, so **no baseline moved** — `planCampaign` never calls `resizeMarchOver`. **Driven in a real browser** on 0024's account at 1 512 px: the sweet spot draws at 5 698 946 with EMH 17 · ED 19 · WE 44 · SG 16 · BB 21 (experiment 124 §E to the unit), taking Spearman II out answers **5 255 703 for 2 257 200**, the pool bar reads **5 040 of 5 600 leadership** and the pane writes *“It fields 90 % of your leadership: the rest bought no damage and cost silver.”*, console clean. **Change 3, asked for the same day** (*“do change 3 too”*) **and built**: `MarchWithin.putBack` carries his rates, and where **nothing wins outright** the dial may take a fill that merely trades — the exact arithmetic `putBackOn` applies to a stop at Generate time, said about a leadership fill: recovers faster, scores ≥ 0 on `silver %/5 + queue %/10 + damage %`, loses at most 3 %, burns no more hired. **A win is never given up for a trade.** It adds **6 more of the 102** benchmark edits (worst −2.7 % damage for 10 % of the silver and 10 % of the queue) on top of the 9 wins, so 15 of 102 presses now leave leadership unspent. **It discloses**, which is the whole reason it may be made on his behalf: `ResizedMarch.traded` carries the cost and the pane states it — *“It fields 90 % of your leadership, at your own rates: 2.2 % less damage for 8.9 % less silver and 8.3 % less training.”* — against the win's own sentence, which says the gap was free; both readings have a test. The dial's contract was **split rather than loosened**: with no policy passed a taken fill still never gives up damage and never carries `traded`, and a second test holds every trade inside the rates. **Gate after change 3**: `tests/engine` **17 failed | 479 passed**, the same 17 and the same three files; `plan-resize` 27/27; `pnpm vitest run src` 592/592; `pnpm e2e` 53 passed; the benchmark payload still moves only `run` and `planMs`. **Driven** on 0024's account at 4 000 leadership, whose sweet spot trades on its first press: Rider III out answers at **3 600 of 4 000 leadership** with that sentence, console clean. `docs/plans/the-re-size-objective.md`. |
| S-120 Monsters as a resource: where gold stops paying, and a dial for it | backlog — measure first | owner, 2026-09-22: *“right now monsters are usually revived as highest troop, and usually they're revived with gold or using dragon coins. So we might need to explore if there is potential better scenarios when varying dominance same as leadership, or with changing the stack… we might need another way to tweak how much gold the user is willing to spend per march. For now I have to lower dominance to do that, but I'm unsure if there's a peak in gold usefulness somewhere down the line.”* **What is already true, so the story starts from it.** The search *does* vary monster counts: `rankHired` has read `pool !== 'leadership'` since S-96, and a dominance type with no cap is sampled against its own whole pool (`housing.dominance / cost`), down the same `{max, 0.7, 0.45, 0.2, 0}` grid the mercenaries walk. What it does **not** do is *rank* on what a monster costs: the search scores damage, silver and `mercLost`, and `mercLost` counts the **authority** pool alone (S-102) — so the Lair's dragon coins and, under any plan but a retrain, the Temple's gold are printed and never weighed. Nothing pushes the plan to field one monster fewer, which is exactly why the owner has to lower the dominance pool by hand to make it. There is no gold budget either (`CampaignInput` carries `silverBudget` and no twin), and the pools themselves are never proposed: `request.housing` is input, and only **leadership** has a fill that may leave it part-spent (S-117, `CAMPAIGN.editFills`). **What has been measured, and what has not.** The dominance *pool* has been swept before — experiments 121, 123 and 125 walk it from 0 to 3 200 — but always to ask which rows the bar carries, never what the gold buys. Experiment 126 (2026-09-21) measured gold across a bar's stops on the 14 benchmark armies that carry one: it moves steeply (median spread **78.7 %** under a retrain, **75.8 %** reviving the top monster) yet ranks the stops exactly as “Hired lost” does on **14 of 14** under a retrain and **13 of 14** under a revive, because 13 of those armies field no monster at all. **Nobody has asked where the knee is.** **What to do, in this order.** (1) *Measure*: on his own camp, sweep the dominance pool and the monster counts under each recovery plan and plot damage against gold and against dragon coins — his march of 2026-09-21 spends **7 992 gold and 2 520 coins** at 600 dominance for 4.1M, and the question is whether that curve flattens. (2) If it does, give the search **the owner's own rate for gold**, the way `CAMPAIGN.putBack` already prices silver and the queue against damage (`silverPerDamage: 5`, `timePerDamage: 10`), so a monster that costs more gold than its damage is worth is declined by the plan rather than by the player editing a pool. A gold *budget* is the cruder twin and probably the wrong shape: a rate says what the player believes, a budget says what they have left. (3) A **dominance fill** mirroring the leadership one, so a march may leave the pool part-spent with the reason written — `resizeWords` already has the sentence. **Two things that bear on it.** The answer is **plan-dependent**: under “retrain everything” a monster costs no gold at all since 2026-09-21 (silver, coins and queue), so the knee only exists under a plan that revives, which `priceMarch` now prices correctly. And it rides on the one open question of the recovery model — whether a monster's `training.silver` is the price of **one** monster or of **ten** (`engine/recovery.ts`); a screenshot of the Lair's recruit screen settles it, and the whole silver-and-coins half of a monster's price moves with the answer. |
| S-115 The leadership dial — a march that does not fill the pool | retired — measured, not built | owner, 2026-09-20: *“we were supposed to also explore using a bit less leadership, if the damage is still good and the ratios are better.”* **Measured** (investigation 0023, experiment 118, `tools/theorycraft/out/118-less-leadership.md`). **On an army that hires nothing the dial is exactly flat** — 0.56 damage a silver at 100 %, 75 %, 50 % and 20 % of the pool — so it buys a smaller bill and never a better rate. **On an army that hires it depends on one number**, and the number is closed-form. The hired count is the smaller of the **stock** the account owns and the **shelter ceiling** `floor((troopFloor − 1) / hpPerUnit)`; the ladder's rungs scale with the fill, so the ceiling does too, and the fill where it stops clearing the stock is **`(stock × hpPerUnit) / troopFloor`**. Above it the dial is free of hired cost — the owner's live account keeps every one of its 83 hunters down to 75 % of the pool and goes **0.86 → 0.98 damage a silver, up 14 %**, for 85 % of the damage. Below it every point of leadership takes mercenaries with it — and gives their **stock** back, 9 hired lost → 8 at 70 % → 6 at 50 %, at a *better* damage a hired unit (323 582 against 298 414: the last three hunters of a stock of 83 cost a whole chunk of ten for 3.6 % more damage). Predicted 72.4 % against measured 75 % / 70 %, and the ceiling is exact to the unit at every fill where it binds (80, 74, 68, 57). **The other army says the opposite**: the 2026-09-17 export's 129 hired units already sit under its lowest rung at the full pool, crossover **100 %**, mercenaries carrying 82 % of the march — its best rate is at 100 % and 97 % of the leadership costs 13.6 % of the damage, so the dial must not be offered there at all. **The shelter is never the thing traded away**: sized by the raw ladder instead, the same 70 % march puts the hunters at the head of the kill queue and loses 2 685 730 damage for nothing — 5 419 461 sheltered against 4 151 856 bare, **+30.5 % on the same 5 466 200 silver and one chunk of stock less**, which is goal 1 in one figure (§1). Wants S-113's table first, and **moves a benchmark baseline**, which is the owner's to register. `docs/plans/total-optimization-as-the-one-answer.md` §5. **Retired 2026-09-20, by the measurement that was meant to size it** (experiment 119, `tools/theorycraft/out/119-dial-against-the-stops.md`), answering the owner's own question: *“offer one more stop on the slider for lowering leadership? Or just take the wins if it's in a small percent of the closest stop there is on it?”* **Neither.** Turned on **every stop of every bar** — 12 stops × 10 lower fills — the dial produced **0 dominations**: not one fill gave at least the stop's damage for no more silver and no more burn. On a stop it is a *bad* trade, because a stop's troop set is already a subset and shrinking the pool re-shuffles its rungs: 97 % of the leadership costs **8–9 % of the damage for 2.5 % of the silver** on the live account's four stops. The two marches that came within 2 % (the export's *more mercs* and *all in* at 97 %) buy **+1.2 % of rate for −1.9 % of damage** — too small to spend a control on, and too visible to swap in silently. And the dial adds nothing the bar does not already have: on the live account the bar's own **silver-saver reaches 1.03 damage a silver against the dial's best 0.99, at 4 hired lost against 8**. 118's *+14 %* was real but measured against the **Tier ladder's** full-pool march (0.86) — the plan already turns that into 1.03, so the finding is **ammunition for S-113's table**, not a new control: *the plan's cheapest stop beats the best you can get by simply fielding fewer troops, by 3.4 %, at half the hired burn.* The crossover formula and the +30.5 % shelter figure stay where they matter — goal 1 (§1) and investigation 0023. |
...
- 2026-09-20 — **S-112 the plan block speaks the army's own resources, and the recap says what a hired unit
  ...
  noughts, a **Per hired** column of dashes, ends reading *"Fewest hired lost"*, a curve column headed
  *"A mercenary"*, a tip printing *"0 gold a march"*, and `bindingSentence` answering *"Nothing binds yet"*
  on an army sized to the last point of its leadership. One reading now decides all of them
  (`spendsStock`, asked of the bar rather than of the stop), so a block either speaks of a stock throughout
  or never. Gold and the dragon coins stay off the table on purpose: a seventh column was measured at 505 px
  in a 462 px pane and cut in 2026-09-16, and the gate of this story is that the hired bar is untouched —
  driven in a browser on both kinds of army to check exactly that. **And the recap's hired line changed on
  his word the same day**: the share of the account's whole stock was a fact about the account, where
  *damage a hired unit* is a fact about the march and the one the bar's own *Per hired* already prints. They
  agree to the unit — `4 · 431.78K a hired unit` on the card against `431 781` on the bar. It follows a
  put-back or a leave-out as well as a Generate, which is what he asked for and what its test holds.
- 2026-09-20 — **S-111 the plan answers for an army that hires nothing, and the fallback it replaces was
  measurably wrong.** He asked for the troops-only frontier to be built, and set the bar for it: *"if we KNOW
  for sure it's the best play with the given configuration and we've explored all possibilities, then we've
  answered the complete optimization goal… check first it's actually the case."* It was not. The Tier ladder
  march — the one Generate answers a hired-free account with, and the one a silent fallback would have handed
  back as *complete optimization* — is **rank 14 of 1 023 subsets on damage** and rank 1 on damage a silver,
  with a **25-march frontier** between the two readings (experiment 115, every subset enumerated; a hill-climb
  past the sizer's flat profile reaches 3 047 818 against its 2 710 128; a brute force over 635 376 count
  vectors finds the sizer optimal *at its own price* and 75.8 % of the unbounded optimum). `planTroopsOnly`
  searches **tier windows ∪ greedy backward elimination ∪ the whole army** — the two families experiment 116
  measured as containing the true optimum on every army small enough to check, where the search's own prefix
  family reaches 91.6 % and, on a 24-type account, 38.9 % — and returns three stops ordered on **silver**,
  which is the axis the stops differ on once nothing drains. Held by a test that enumerates every subset at
  three leadership pools and asserts the dearest stop **is** the optimum. Nothing that answers today can reach
  the new path, and the benchmark payload is byte-identical on every field but `run` and `planMs`. What it
  exposed is the next plan: the bar still says *"spending 0.0 of the hired stock"* to an army that has none
  (`docs/plans/total-optimization-as-the-one-answer.md`).
- 2026-09-19 — **S-110 Complete optimization leads the Battle card; its default is measured and left
  open.** *“Also reorder complete automatization to first and default. And the button to change stacking
  algo should be more visible on mobile.”* The **order** is the card's own and cost nothing: the plan is
  first in `METHOD_CHOICES`, the stored ids and `METHODS` are untouched, nothing migrates. The **default**
  was tried and measured: with `buildSetup` on `plan`, a first-run profile — which holds no mercenary —
  answers *“There is no campaign to plan from this army…”* to its opening Generate, because the plan plans
  by spreading hired stock and there is none to spread. That is **26 of the built app's e2e journeys**,
  J1 included. The owner was shown the three ways out and chose to leave the default on the tier ladder
  and keep the question open; the reason is written at the line in `state/defaults.ts`, and the trap that
  comes with the flip is written beside its test: `SHARE_SETUP_TEMPLATE` must be **pinned at `elite`**
  when the day comes, because what a stripped link leaves out has meant the tier ladder since v1. The
  **button** that opens the folded method list is a bordered, full-width, 44 px button with a chevron
  now, in place of 26 px of brass text that read as a caption on the card above it; it is `ChoiceList`'s,
  so the Recovery plan's fold gained the same one.
- 2026-09-19 — **S-109 a finished run opens the phone's March sheet, and a swipe down closes it.** *“On
  mobile, generate should open recap by default when finished. Also it would help to be able to close it
  with a gesture like sliding down.”* The answer to a press on Generate is the sheet arriving, not a line
  in the bar and a second tap to read it: **J1 on a phone is two taps** where it was three, against a
  budget of three. It opens only where the March *is* the sheet (below 1200 px), only on a run — a
  re-size keeps its snapshot's stamp (S-104), so a put-back made inside the sheet does not re-open it —
  and the width is read at the moment the run lands, so a window narrowed afterwards raises nothing. The
  bar's spoken sentence and its pulse are untouched and still right, the sentence being written only
  while the sheet is shut. The gesture is Mantine 9.6's stock `useDrag` on the sheet's header, under
  M3's drag handle: past 96 px or on a downward fling it closes, otherwise it springs back. It moves the
  sheet on the CSS **`translate`** property because `transform` on that element is Mantine's, written
  through React and transitioned over 250 ms — a transform here would be overwritten *and* a
  quarter-second behind the hand. `touch-action: none` on the header is what keeps the touch from
  becoming a scroll. Fixed on the way: the sticky header wore Mantine's `--mantine-color-body` and drew a
  band across the top of the sheet in the dark scheme; it wears the sheet's own `--pyr-pane` now.
- 2026-09-19 — **S-108 the recap's ratios are on the worst opening, like the bar's.** *“Damage/silver
  differs in the plan table and in the battle summary.”* Two arithmetics on one screen:
  `simulateBattle` computes its three ratios from **`avgDamage`**, the midpoint of the two openings, and
  the recap printed them, while the plan's trade has divided the **worst opening** since S-94. Measured on
  his usual setup's steady max: **1.3928** a silver on the recap against **1.3553** on the bar, and
  **2 879.30** a dragon coin against **2 801.79** — his own screen read 1.40 against 1.36. **The midpoint
  fields stay**, because two things depend on them: `tests/engine/summary.test.ts` pins each at
  `avgDamage / cost`, which is TotalStack's own reading of its Battle Summary, and `src/engine/search.ts`
  optimises `damagePerSilver` when the player picks it as an objective. So the **recap** moved instead, to
  the two figures already on its card — `minDamage` two rows above it and the recovery bill — through
  `src/ui/sections/march/worst.ts`. The same story for the Details fold: `summary.damageByPool` is the
  midpoint, so *Damage a hired unit* was one arithmetic over another; the split is summed from the
  **enemy-first journal**'s army lines by pool now, which keeps the ratio one line of the block over another
  and makes it the bar's own `repeat.hiredDamage / repeat.mercLost` (design rule 5, one name one thing).
  **Nothing in `src/engine` changed and no plan figure moves.** Held two ways: `march.test.tsx` reads the
  rendered *Damage per silver* and *Damage per dragon coin* against `minDamage / cost` and asserts on the
  same march that the midpoint field is still the larger number — so a return to it is caught, not just a
  drift — and `plan-criteria.test.ts` calls the recap's own functions on every stop of all seventeen
  benchmark armies and holds all three ratios to the bar's to the third decimal.
- 2026-09-19 — **S-107 taking a type out re-derives the hired count up to what the shelter allows.**
  *“Taking out one group, like SP1, doesn't compute again the mercs and I'm left with a merc stack that's
  below what could be added with proper shielding.”* S-104 capped every authority type of a re-size at the
  **selected stop's own count**, on S-80's argument that a count which can only fall keeps the rest of the
  plan affordable. It does — and it also makes his edit a no-op: taking a troop type out hands its
  leadership back to the stacks that are left, the troop floor rises, and the count the stop stood under the
  **old** floor is not what the new one shelters. **The cap now** is what the account can spend on every
  march the stop plays — `largestSustained(stock, repeats)`, the arithmetic `planCampaign` has always
  anchored a hired type on (`largestFor`), exported for this — or the whole authority pool for a type hired
  with no cap; a dominance type keeps its pool bound (S-102). The stop's own count is a ceiling nowhere; the
  sizer and `shelterUnder` decide the count, so a **put-back** still re-derives downward as it did.
  **His 450-hunter camp**, sweet spot, Rider I taken out: the same four troop stacks either way
  (Archer II 1 625 · Spearman II 1 290 · Rider II 660 · Rider III 370) for the same **2 635 500** silver and
  the same 12 d 10 h of queue, the floor **456 624 → 696 340** HP, the hunters **38 → 72** — the sheltered
  maximum to the unit, one more standing over the floor — and the damage **1 902 783 → 2 452 872**,
  **+28.9 %**, the burn 4 → 8 chunks. The cap it is handed is `largestSustained(450, 3)` = **374**.
  **His TotalStack profile**, Archer I out: floor 319 260, hunters **49 → 62**, damage
  **2 016 985 → 2 163 313** (+7.3 %) for the same 2 234 900 silver and queue, the burn 5 → 7, its four
  tier-3 monsters unmoved at 10 · 4 · 3 · 3 for 960 coins. **And downward still works**: Archer I put back
  on the first camp lowers the floor to 336 878 and answers **35** hunters for 1 987 686 under both cap
  rules, to the unit. **One army pays a little for it**, disclosed rather than hidden: on his usual setup
  the same edit fields 19 → 23 hunters and the damage moves 2 522 521 → **2 518 620**, −0.15 %, for one
  more chunk burned — a bigger hired stack is killed a slot earlier and strikes once less. The pane's line
  says what happened — *“… nothing else was pushed out, and your hired stacks are re-sized to what the
  troops shelter”* — and a type the bound puts at nothing is named in its own words (*“your … stock cannot
  last every march of this stop”*) rather than S-104's *“this stop spends no …”*, which is no longer the
  reason. `tests/engine/plan-resize.test.ts` holds it on every stop of every benchmark army: the bound is
  never tighter than the plan's own count, the answer fields at least what the stop did of every hired type
  unless the shelter or the camp's housing forbids it, and every stack is still strictly under the lowest
  troop stack — with the two camps' figures as a directed test. **No plan figure moves**: the re-size is
  outside `planCampaign`, and the benchmark run after it is byte-identical to snapshot 22 on every field but
  `run` and `planMs`, so no snapshot 23 was written.
- 2026-09-19 — **S-106 the stop rules judge on figures, so the savers come back.** *“It seems the last
  change made us lose some of the stops on the slider. I only get sweet spot and steady max in my usual
  setup.”* **The cause.** S-105 redefined *damage a hired unit* as the hired stacks' own damage over the
  chunks they cost — right for the column he reads — but **five rules that drop a plan read that ratio**:
  the silver saver's `beatenOnBoth`, the efficient rungs the sweet spot's knee is read off, the
  `middleOfRange` tie-break, the `more-mercs` tie-break and the S-99 cut. A pair of ratios is not a
  domination test: both carry the same numerator, so a change in **how the damage is attributed** re-orders
  rungs on ladders where not one march moved, and five of the sixteen benchmark bars moved with it.
  **The rule now**: a march is beaten only when another has **at least its damage, at most its silver and at
  most its burn**, one of the three strictly better (`beatsOnFigures` — S-93's `tighterShape` reading and
  S-94's all-in offer's, the one the criteria are already written in); the two tie-breaks use the
  **campaign's** damage then its silver. `bestFor` and the printed *Per hired* keep S-105's ratio, which is
  what he reads. **Snapshot 20's bar comes back on all four armies that lost something**: the monster camp
  3 → **4** stops, its *more mercs* back and its sweet spot 93 298 414 → **89 196 808** at 24 chunks
  (snapshot 20's figure to the unit); the evening account 4 → **5**, a silver saver again; his 450-hunter
  camp 3 → **4**, likewise; his live camp of 2026-09-18's *more mercs* back at 37 chunks,
  11 029 591 → **12 086 359**. **Four bars differ from snapshot 20, each on the new rule and each a better
  plan.** The 7 000 export's *more mercs* stays at 47 chunks rather than 44: the two rungs are exactly as
  near the middle and the campaign tie-break takes **21 363 106** over 20 079 262 for the same silver. The
  4 000 case's sweet spot moves to the top rung — 8 084 653 → **8 222 546** for the same 6 083 200 — and the
  rung it left becomes a **new silver saver**, 5 212 861 for **3 820 600**, 63 % of the silver. And the
  savers on the 12 000 export (2 614 000 → **2 477 300** silver) and his live account at 20 000
  (4 414 200 → **3 602 400**) step left, the weaker test no longer throwing those savings away.
  **The S-99 cut keeps its four ratios, and the measurement is the reason**: asked as figures it is never
  earned on the monster camp — the strongest full-set campaign there is 66 409 153 for 25 764 300 silver,
  24 chunks and 31 680 coins, while the prefix-10 family reaches **97 458 367 for 36 436 800, 32 and
  28 200**, half again the damage for 41 % more silver, and the cheapest prefix campaign that out-damages
  the full set costs 27 396 900 — so nothing of it is behind on none of the four, S-58 B asks for all
  fourteen types again and the camp's sweet spot falls **93 298 414 → 66 409 153** with its coins rising
  28 200 → 31 680. That rule compares two **families** at different places on the frontier, which is the one
  comparison in the file a domination test cannot make; the note beside `readings` in `plan.ts` carries the
  numbers, and this is the one part of the brief that could not be made to hold.
  **Scenario 17 is his usual setup** — Aydae 43★3 alone, 5 200 leadership / 2 000 authority / 200
  dominance, guardsmen I–III with the top melee and ranged tiers clicked out, specialists I, the monster
  window on tier 3, Epic Monster Hunter VI ×90, temple 0, and the export's own bonuses (of its eight
  permanent rows only **Army Modernization** holds values, +2 health on melee, ranged and mounted, which is
  his page's *“Other: 2 of 8 sources on”*). Its steady max **is his screen's march to the unit** — Archer I
  1 313 · Rider I 654 · Archer II 726 · Spearman II 725 · Rider II 361 · Rider III 203 · Epic Monster
  Hunter VI 32 · Water Elemental 18 · Battle Boar 8 · Emerald Dragon 7 · Stone Gargoyle 6, for 2 232 600
  silver, 2 976 gold, 1 080 dragon coins, 7 d 11 h and 4 hired lost — at **3 025 937** worst against the
  3 041 561 his app printed, **0.51 % under**, a bonus his live account carries and this export does not.
  Pinned at what it measures: **2 stops**, `damageFloor` **0.98** (11 721 371 against the best sizer
  sequence's 11 892 528, the narrowest in the file), `winsHired` true (422 679 a chunk against 307 403).
  **It still shows two stops, and that is its ladder and not a defect**: the burn ladder is **1 · 2 · 4** and
  nothing else, so no rung stands strictly inside the gap between the knee and the top for a *more mercs*,
  and its thrift rung is **21 % less efficient a silver** than the sweet spot (1.256 against 1.440 —
  2 % of the silver saved for 15 % of the damage), which the silver saver's own rule refuses. Before S-105
  it carried three, and the third was not a saving: the ratio rule threw the **top** rung out of the pool
  the knee is read off, which put the recommendation on the thriftiest march of the three — 10 136 819
  against the **11 417 051** the knee recommends now, 11 % less damage for 1.5 % less silver. S-106 keeps
  the better recommendation and does not manufacture a stop to sit under it. **A criterion of its own**
  holds the rule — *a bar with a saving on it carries a silver saver*, and the saver is the **cheapest**
  band plan left of the sweet spot that is at least as efficient a silver and that nothing beats on the
  figures — failing on HEAD (987784b) on the 12 000 export and on his live account at 20 000 with the two
  savings above, and passing on all seventeen after. It is **vacuous** on the two armies that lost their
  saver outright, and that is worth knowing: there the ratio rule moved the *sweet spot* one rung right, and
  a dearer sweet spot raises the efficiency a saver must clear above every cheaper plan — what catches those
  two is the benchmark's `stops` pin. Snapshot
  `tools/theorycraft/out/benchmark-2026-09-19-22-stops-on-figures.{json,md}` and the proposal regenerated
  with `registeredBy: null`; **nothing re-based** — only the owner re-bases.
- 2026-09-19 — **S-105 “damage a hired unit” is the hired stacks' own damage per hired unit lost.** Two
  reports in one day about one column: *“it says over a million but in total they do less than 1M”*, then
  *“dmg per hired is still broken: it shows a damage per hired almost above total damage”*. He was reading
  the arithmetic exactly: `damagePerMercenary` divided the **whole** march's (campaign's) worst opening by
  `mercLost`, the chunks of ten the authority pool loses for good, so on a march whose troops and monsters
  do the hitting and whose one hunter stack loses a single chunk the column printed nearly the damage
  column again. **The definition now**: `marchOf` sums the enemy-first journal's army lines that belong to
  an `authority` stack — one pass over entries it had already built, no second battle — and returns
  `hiredDamage`; `PlanMarch`, `PlanRepeat` and `PlanTotals` carry it (the campaign's is the repeats plus the
  finale, or every march of a `sequence`; the troops-only tail adds none, fielding no hired stack), and
  `damagePerMercenary = hiredDamage / mercLost`. **The hand check**, three marches stack by stack, each
  equal to the engine to the unit: his **TotalStack profile**'s silver saver stands Epic Monster Hunter V
  ×10 at kill position **10 of 13**, striking **3** times at 56 280 for **168 840**, losing **1** chunk —
  **168 840** a hired against the old **1 000 201**, on a march whose whole worst opening is 1 000 201; the
  **450-hunter camp**'s sweet spot stands Epic Monster Hunter VI ×48 at **6 of 6**, **2** strikes at
  776 597 for **1 553 194**, **5** chunks — **310 639** against 484 660; the **monster camp**'s steady max
  stands hunters ×69 at **17 of 19** (4 × 993 096) and Bear V ×4 at **19 of 19** (5 × 149 600) for
  **4 720 384**, **8** chunks — **590 048** against 3 222 993. Campaign-wide the same three read 131 856
  against 702 728, 309 201 against 510 146 and 543 725 against 3 045 574. **Every rule that says *a hired*
  reads the one figure** (design rule 5): `bestFor.hired`, the trade's *Per hired*, the recap's *Damage a
  hired unit* (the 👑 line over the hired units lost, two figures already on the same card), the reference
  table's *A mercenary*, the silver saver's `beatenOnBoth`, the efficient rungs the sweet spot is read off
  and its tie-break, the `more-mercs` tie-break, the S-99 cut's four readings, the search's `mostThrifty`,
  and the benchmark's `a hired`, `a soldier` and `a monster`, each now its own group's share of the same
  journal the damage column is summed from. The **undominated frontier is not**, and the definition says so:
  it is ordered on silver, the burn and total damage — three figures, not a ratio. **The bar moves on five of
  the sixteen armies**, because the rules that pick a stop read that column: the monster camp loses
  `more-mercs` (4 → 3 stops), the evening account and his 450-hunter camp lose the **silver saver** (5 → 4
  and 4 → 3), and the 7 000 export's and his live camp's `more-mercs` land on a different rung (20 079 262 →
  21 363 106 for 47 chunks instead of 44; 12 086 359 → 11 029 591 for 34 instead of 37). The other eleven are
  byte-identical outside the three ratios. **`bestFor.hired` moves on one**: his 2026-09-17 export at 12 000,
  silver saver → sweet spot. **Per-scenario, the best stop's damage a hired, old → new**: bear ×1
  18 189 008 → 112 200, ×2 9 206 704 → 168 300, ×3 6 250 003 → 224 400, ×10 5 223 344 → 776 050; the e2e seed
  1 146 218 → 423 145; the monster camp 3 716 534 → 597 802; the 4 000 case 425 508 → 293 176; the 7 000
  export 537 039 → 357 360 and at 12 000 470 843 → 333 911; his live account 1 130 835 → 317 110 and its
  evening form 461 316 → 299 381; *Aydae alone* 419 836 → 300 662; his live camp of 2026-09-18
  472 637 → 233 912; his 450-hunter camp 630 122 → 309 201 and the 120-hunter reading 406 254 → 258 218; his
  TotalStack profile 702 728 → 131 856. **Nothing was re-based** — only the owner re-bases — so
  `pnpm vitest run tests/engine` ends on **17** failures where it began on **8**, and every new one is a
  figure that reads *a hired*. Unchanged: the 7 000 export's `stops` (4 pinned, 5 offered), the 12 000
  export's (5 pinned, 3), his live account's `externals.damageFloor` (29 743 332 against 29 806 888.8) and
  the 7 000 sweet spot's campaign floor (18 796 348 against 21 600 000). Worse: the monster camp's `stops`
  (5 pinned, 4 → **3** offered). New and **falling**: the e2e seed's Total-Optimization floor on *a hired
  soldier* 1.1120 → **1.0410** (1.11 pinned), the 4 000 case's 1.2185 → **1.1202** (1.21), his live camp of
  2026-09-18's 4.2076 → **1.9453** (4.20); the evening account's `stops` (5 pinned, **4**) and his
  450-hunter camp's (4 pinned, **3**); the criteria's `leastPerHired` on three armies — the engine tests'
  army 440 274 → **340 160**, the 7 000 request-builder case 521 449 → **381 901**, the 12 000 export
  645 859 → 487 046 → **321 401** (its two neighbours' `sweetPerSilver` failures of 1.1830 and 1.6319 are
  unchanged and now masked behind the earlier line); and `plan-shape`'s `mostThrifty.damagePerMercenary`
  661 893 → **377 848**. New and **rising** but pinned exactly, so red all the same: bear ×10's `winsHired`
  and his 120-hunter camp's and his TotalStack profile's `externals.winsHired` are all **true** where
  `false` is registered — the plan now gets more out of a hired unit than the sizers and than the other
  calculators on those three. **Cost**: nil in work — no battle is run that was not run before — and the
  benchmark's own `planMs` over the sixteen armies reads 22 754 → 24 585 ms, inside a loaded machine's
  run-to-run noise and concentrated on the armies whose bar moved. Snapshot
  `tools/theorycraft/out/benchmark-2026-09-19-21-hired-damage.{json,md}`, the proposal regenerated with
  `registeredBy: null`, and the review page and its stage history rebuilt on it.
