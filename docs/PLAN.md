...
| S-113 The insight table: one rate the player switches, and the best of every column marked | done | owner, 2026-09-20: *“a clear table telling you the facts summed up and letting you choose easily and switch between objective with added knowledge.”* **Measured before it was drawn** (experiment 120, `tools/theorycraft/out/120-what-the-table-should-carry.md`, over the 16 benchmark armies the baseline is registered on), because S-113 is a **layout** story before it is a data one: `docs/design.md` §7 hard-codes the column list and a seventh head measured **505 px in a 462 px pane** and was cut. Two questions decided the build. **Is a best mark worth its ink?** The bests land on **2.38 different stops on average** and one stop wins everything on only **3** armies — the ones whose bar is one or two stops long — so the marks are the table doing its job rather than a ranking with extra steps. **Which rate earns a column?** Counted as *“how often is this the only fact naming its stop”*: **Per gold is sole on 7 of 16**, *per hour of queue* on **0** (it always agrees with Silver or Per silver), *per dragon coin* exists on 2 armies and is sole on none, and **Silver, Queue and Hired lost never disagree with each other at all**. **So the two rate columns became one with a switch over it** — Per silver · Per gold · Per hired, each offered only where the army spends it (design rule 15), no control at all where there is one rate — which is the owner's *“switch between objective”*, costs no width and **gives back the column that lets Per gold exist**. **The best figure in each column is marked**, weight first and the anchor's ink second (design rule 24: colour is never the only signal), with a `Table.Caption` saying what the mark is; nothing is marked on a one-row bar, where every row reads the same, or where two rows tie at the top. **Never on Per hired** — 0019 §2.3 measured that ratio rising while the march collapses. The marks are in each row's accessible name except the silver rate's, which the note under the name already says: experiment 120 asked the engine's own `bestFor.silver` and the column's best cell of all sixteen armies and they named the same row **16 times of 16**. Verified in a browser on the owner's seeded army — **463 px table in a 463 px wrapper, no overflow on either rate**, and Per gold's best cell (15 161 against 12 951 · 13 795 · 12 328) on a row no other column names — and held by J6 at phone 390×844 and desktop 1400×900. `docs/plans/total-optimization-as-the-one-answer.md` §3. |
