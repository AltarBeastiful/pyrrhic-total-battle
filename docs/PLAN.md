...
| S-115 The leadership dial — a march that does not fill the pool | backlog | owner, 2026-09-20: *“we were supposed to also explore using a bit less leadership, if the damage is still good and the ratios are better.”* **Measured first** (investigation 0023, experiment 118, `tools/theorycraft/out/118-less-leadership.md`): every march the app has ever offered fills the leadership pool to its last point, and nothing had asked what the same march is worth fielded smaller. **On an army that hires nothing the dial is exactly flat** — 0.56 damage a silver at 100 %, 75 %, 50 % and 20 % of the pool, damage and silver scaling together to the tenth of a per cent — so it buys a smaller bill and never a better rate. **On an army that hires it pays**, because mercenaries cost authority and troops cost leadership: the hired damage does not move while the troops' bill falls with them. The owner's live account keeps **95.2 % of its damage for 92 % of its silver** at a 92 % fill (0.89 a silver against 0.86, **625,000 silver a march**), and **0.98 — up 13.3 % — at 75 %**, for 85 % of the damage. **The guard the story cannot ship without**: between 75 % and 70 % that account's hired damage goes **2,685,730 → 0**. The ladder sizes every rung to nearly the same HP (837,840 … 826,198), so the whole troop wall crosses the hunters' 798,626 at one fill, the hunters become the biggest stack on the field, and **the biggest stack strikes zero times** — 38 % of the march's damage for a 6 % saving, from 83 units that were carrying 47 % of it. The floor is closed-form, **`fill ≥ hiredTop / troopFloor`** = 72.4 % here, and bisection to the tenth of a per cent finds the cliff at 72.4 %: the engine can compute it before it offers anything, and an army whose hired stacks already stand above the lowest rung (the 2026-09-17 export: 1,331,818 against 361,200) has no floor to fall through at all. **Not a sixth stop on the bar**: hired units lost is 9 at every fill on the live account and 27 on the export, so the dial moves silver and queue time and does not touch the burn axis the bar is ordered on — a second dimension, not a further point along the first, and on the joint (damage, silver) frontier it supplies 12 of 14 undominated marches while the plan's own stops still hold both ends. Wants S-113's table first, and **moves a benchmark baseline**, which is the owner's to register. `docs/plans/total-optimization-as-the-one-answer.md` §5. |
