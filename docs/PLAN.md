...
| S-65 Regression on the criteria, and the refactor | done | owner, 2026-09-18: *"check we have sufficient tests to avoid regression in the future. Always test the actual criteria dmg/silver dmg/merc not the shape of the march. When done enter a refactor phase to avoid unused code, broken code or unnecessary complexity."* **Tests:** `tests/engine/plan-criteria.test.ts` freezes the bar's criteria as floors — the thrift end's damage a hired unit, the sweet spot's damage a silver and a hired unit and its campaign damage (with a ceiling on its silver), the most-damage stop's damage a march and a silver, the plan's campaign damage — a tenth of a percent under what was measured on 2026-09-18, on the engine tests' army and, where the file is, on the owner's export at 7 000 and 12 000 leadership (skipped elsewhere); it also holds the criteria's own consistency: burning more buys more along the bar, the thrift end is the bar's best a hired, the most damage its best a silver, the sweet spot beaten on both ratios by neither end. **Refactor:** the silver axis, the near-stop merge, the fillers and `step`, `best-for-silver`, the `alternatives` cap and `planAlternatives`, `CAMPAIGN.planBar`, the unread `CampaignInput.objective` and a duplicated ratio helper are gone from the engine; the bar is three stops along hired burn, and `PlanPick` has three values. The UI lost the axis props and words, `deltaRatio`, and eleven exports nothing imported; the march barrel re-exports only what the shell reads. Experiments that passed the removed inputs were trimmed (91 keeps its three-way output of 2026-09-16 as `out/91-cross-review-2026-09-16.md` and crosses the S-58 flags on the one bar now; 92 draws the one axis). The S-58 flags and `sizerShape` stay as inputs because 80, 81, 92 and 93 are their record. 54 files, +266 −697. |
