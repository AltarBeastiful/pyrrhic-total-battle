...
| S-69 Four stops along the mercenaries, the sweet spot at the knee; the benchmark of the three methods | done | owner, 2026-09-18: *"best optimization still doesn't offer enough splits"* and *"in my mind we would have on the slider: least silver, sweet spot, more mercs, most mercs — whilst still trying to aim for an ok sil/dmg and merc/dmg on each"*. Measured first (`out/99-three-stops.md`): on his latest export silver is flat from 7 to 10 burned — the troop ladder is sized off the biggest hired stack and the other types ride for free — so the middle-of-the-range rule landed on 8, one unit from the thrift end, while 10 burned buys 8 % more damage for the same silver, is the best damage a silver, and is the knee of damage against burn. The four stops (`PlanPick`): **least silver** (the efficient rung costing the least, fewest burned on a tie), **sweet spot** (the efficient rung farthest above the chord over the whole ladder — the knee; the middle rule when there are fewer than three rungs), **more mercs** (the rung nearest the middle of the gap to the top), **most mercs** (the top: the most the troops shelter, the most damage). His latest export draws 7 · 10 · 12 · 13 at 5 064 553 / 5 862 857 / 5 986 678 / 6 245 103 for 3 619 200 / 3 619 200 / 3 906 000 / 4 551 300 silver. The criteria floors are re-based per stop. **The benchmark** (`tools/theorycraft/100-three-methods.test.ts`, `out/100`): Tier ladder, Troops first — each as the plain sizer and as Generate runs them on his setup, the priority search on average damage and on damage a silver — and every plan stop, all played for the same four marches, the sizer methods re-sized each march on the stock the last one left. On four-march damage the plan's most-mercs stop wins at 12 000 leadership (28 812 258 against Troops first's 27 838 482) and **loses narrowly on the two other cases** (24 496 571 against Troops first's 24 823 960 at his setup; 24 814 601 against Tier ladder's 25 952 553 on the 2026-09-17 export), each time because the re-sized sequence burns 24–45 % more of the stock (62 and 93 hired against 50 and 64) — a descending sequence the plan cannot express, since it repeats one march and plays a finale. On the owner's two ratios every plan stop beats every sizer sequence: the sweet spot 1.58 a silver · 569 508 a hired over the campaign against the best sizer's 1.42 · 400 386. Open: a descending-sequence shape for the plan (march 1 on the whole stock, each next on what is left), which would let the plan claim those last 1–5 % of damage when the player wants them. **Amended the same night, from the owner's live account** (one hired type, 83 in stock, 20 000 leadership, seen in his own browser: two stops, 5 and 7 burned, 7.1 M and 7.7 M): the grid samples each hired type at five counts and the climb walks around the winner, so no rung stood between. Three fixes in the search: **(1)** every burn level under the winner's gets a swept vector (the winner's scaled to it, each type rounded up to a chunk); **(2)** each swept vector gets the winner's walk over depth and scale (scored on the coarse grid alone the 60-hunter march came out dearer and barely stronger than the 48, and fell off the frontier); **(3)** a new shape, **the winner's own rungs with the vector's mercenaries** (`WINNER_RUNGS_DEPTH`, shape `winner`) — the ladder shrinks the troops with the hired stack it is sized off, so fewer mercenaries lost twice; the winner's rungs kept whole hit for 7 453 778 at 60 hunters for the winner's own 7 756 500 silver, where the search's best was 7 170 113 for 8 229 200. Two rules tightened: the knee falls back to the middle of the efficient rungs when no rung stands above the chord (the 2026-09-17 export at 7 000 is convex), and **least silver** is the cheapest march of the band left of the sweet spot that nothing beats on both ratios and that costs no more silver than it (absent rather than misnamed; equal silver allowed, the fewest burned winning the tie). His live account draws **4 · 6 · 7** at 6 806 614 / 7 453 778 / 7 745 002 for the same 7 756 500 silver; the exports are unchanged or better; the criteria floors re-based with their notes. |
