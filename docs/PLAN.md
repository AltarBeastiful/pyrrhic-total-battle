...
| S-62 The sizer's own march as a shape the plan can find | flagged | owner, 2026-09-17: *"are we actually exploring what each additional troop type in the march brings us? … adding them back just brings damage up and cost down"* (leadership 12 000, authority 7 000; SP1 and ARC1 left out of the sweet spot). Where the search sizes troops: a **ladder** — the strongest `depth` types by damage per HP, one rung each, 2 % apart, scaled — so it fields a *prefix* of that ranking and never any other subset, and its shape is not the Elite sizer's, which is what the March pane draws after a put-back (`resizeMarch`, elite, the plan's hired counts as caps). `CampaignInput.sizerShape` (`CAMPAIGN.planFixes.sizerShape`, **off**) scores the Elite sizer over every troop type as one more shape a mercenary vector, so a march a put-back would reach is one the search can find. Measured on the 2026-09-13 export (`out/93-sizer-shape.md`, three housings): the flag never changes a stop — with that file's bonuses the ladder over six types beats the sizer over eight (9 383 022 against 8 290 542 at 12 000 / 2 180) and the plan agrees with the sizer on the same types to within 1 % — and costs about 100 ms. **The owner's live account is not in that file**: his linked march hits for 6 133 203 in the app and 4 986 889 under the export, so the flag waits for a fresh export to be measured where it matters. Also his: the bar carries **three** stops from now on (`planAlternatives: 3`, no fillers), and the recap does not show damage a hired unit for the march on screen — backlog. |
