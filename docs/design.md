...
`--pyr-input-bg` / `--pyr-input-bd` are the ground and the edge **a field actually wears**: Paper on a
`gray-4` edge in the light scheme, Anvil on a `dark-4` one in the dark. Not the well of the table above,
which is what the artboards draw (`CommandBar.dc.html`, `.input`: `#101413`) and what `--input-bg` used to
be pointed at — Mantine declares `--input-bg` and `--input-bd` again on every input wrapper, once per
variant, so the value written on `<html>` never reached a field and never has. Anything hand-built that has
to pass for a field reads these two rather than guessing: today that is the phone bar's housing chips, which
looked like black holes beside the same control on a desktop (owner, 2026-09-19) until they did. Putting the
artboard's well back is a whole-app change — 47 fields, `pnpm contrast` and the kit baselines — and waits
...
│                 battle story, and the saved marches (`#march-foot`). What *acts* on the march — copy,
│                 edit, keep, send — is a row of marks on the March's own heading (2026-09-21)
...
**The phone's March sheet is the one surface that arrives and leaves by itself** (owner, 2026-09-19,
S-109). A finished Generate opens it — the answer to the press *is* the sheet — and a swipe down its header
puts it away, past 96 px or on a downward fling. It is one material from its top corners down: the header is
part of the sheet, not a bar of page ground across it, and it carries M3's 32 × 4 drag handle centred on its
top edge as the sign that it can be pulled. The handle is decoration in the accessibility tree (`aria-hidden`):
the Close button beside it and `Escape` are what a keyboard is given, and a gesture is never the only way out
of anything (design rule 24).
...
**Two exceptions, both the owner's, both 2026-09-21, and both inside the same box.** A kind listed in
`GLYPH_ART` is drawn as a **picture** rather than as an emoji — the **dragon coin** is the only one, because no
face in Unicode draws one and 🏵️ was a rosette standing in for the game's own piece of art (64 × 64 PNG,
`src/ui/domain/art/`, published through Vite like any other asset). A kind listed in `GREYED_GLYPHS` keeps the
platform's emoji and loses its **metal**: 🪙 is a gold coin in every emoji font, beside 💰 which is gold too, so
the two purses a march is counted in wore one colour; `filter: grayscale(1) brightness(1.12)` is what silver
looks like next to gold. Neither exception touches the box, the metrics or the accessible name.
...
| `CampaignPlan.alternatives` | **The trade** | The plans nothing else beats on every resource at once, cut to the ones near the goal: the trade between silver, hired stock and damage. **The bar runs along the hired stock** (owner 2026-09-17 — the slider balances burning silver efficiently against burning mercenaries efficiently, and the sweet spot keeps the middle): **five stops at most**, one plan a burn level, **thriftiest first**, so a row's place on the bar is what a march of it burns for good. Its rows are the picks below, and a row is named by **which answer it is** and by nothing else — the plan's name is the row's own head — with a bar as long as the damage a march of it deals, measured against the loudest plan on the list. The **two efficiencies are notes on the stops that have them**, one muted line under the name in the trade's own words ("best a silver", "best a hired", `PlanRow.bestFor`), and never stops of their own: a separate "Best for silver" stop measured as the top stop to 0.2 %, which the owner called inefficient and frustrating. The columns read one march at a time: **Plan** and then **Damage · Silver · [Gold] · Hired · Per silver · Per hired**, each head behind its glyph and the seven-figure columns in compact figures (S-59). **The two purses are two columns** (owner, 2026-09-21): the silver carries the training queue under it and nothing else, and the **gold** — the Temple's price, back as a column after 2026-09-16 cut it — carries one figure and nothing under it. Silver and time are what the barracks charge and what comes back; gold is what the Temple charges and what does not. **The dragon coins are in neither**: they rode under the silver from S-102 and under the gold for an hour the same day, and two muted second lines in a seven-column table is a table read twice — the recap says them with their own mark and their own ratio a screen above (*"we can keep it in the battle summary, it's enough there"*). The gold column is drawn **only on a bar that spends any**, as the two stock columns are (rule 15), and the cheapest stop is marked like every other price. Its width came from the table itself rather than from the pane: the hired head dropped its second word to "Hired", the damage bar went 40 px → 28 px, and the cell padding 4 px → 2 px, which puts the seven columns at **463 px in a 463 px box** — no scroller, which is what `paneFrame().scrollers` in `e2e/generate.spec.ts` holds the pane to — and **Per silver and Per hired are the march's own ratios**, not the campaign's, so every figure on a row is a fact about the one march the recap above is drawing. **The best figure in each column is marked** — heavier and in the anchor's ink, two signals because colour is never the only one (rule 24) — with a caption under the table saying so, and **never on Per hired**: `docs/investigations/0019` §2.3 measured that ratio rising while the march collapses. *(A **Per gold** column was measured as worth its width — experiment 120 found it the only fact naming its stop on 7 of the 16 benchmark armies — and built on 2026-09-20 as a switch over a single rate column, so that three rates could share the width of two. The owner read the switch and said it did not explain itself, so it was reverted the same day: the table keeps its two fixed rate columns, and the measurement waits for a shape that needs no control.)* *(The **Gold** column was measured before it was drawn — experiment 126, over the 14 benchmark armies that carry a bar, under each of the three recovery plans — and the measurement first argued against it: gold moves steeply along a bar (median spread **78.7 %** under a retrain, **75.8 %** reviving the top monster, **33.9 %** reviving everything) but *ranks* the stops exactly as "Hired lost" does on **14 armies of 14** under a retrain and **13 of 14** under either revive, because gold is mostly the hired stock's revival. The owner's answer, the same day: *"monsters are revived using gold for a substantial sum and mercs aren't cheap either if there's many — why not comparing, or at least inform? … using revive the highest elite, it should say the gold anyway, as we're not training, we're reviving with gold."* The measurement answered "does gold rank the stops differently"; the question is what a march **costs**, and **thirteen of those fourteen armies field no monster**. Under the plan a new setup now opens on, a monster stack's price leaves the silver column for this one, and a bar whose stops differ by a monster differs by thousands of gold that nothing on screen said.)* A plan several of the definitions fit wears the first, in this order: **Sweet spot → Steady max → Silver saver → More mercs → All in**. |
| `PlanPick: 'silver-saver'` | **Silver saver** | The cheapest march worth fielding: of the rungs nothing beats on both ratios, the one that costs the least silver — the fewest hired units burned when two tie. The thrifty end of the bar. (Named "Least silver" until 2026-09-18: a superlative read as the cheapest march the app could imagine, where the stop is a march that spends *less*.) |
| `PlanPick: 'sweet-spot'` | **Sweet spot** | **The knee of damage against whatever the bar is ordered by** — where one more of that resource stops buying damage as fast as it did before. That resource is the hired stock burned where the account hires anything, and the **silver** where it does not (S-111/S-112: an army with no stock has a bar of its own, and naming its axis after burn would name the one thing it never spends). Where the bar opens, and its row says so in words. (Re-aimed 2026-09-16: the old rule — the plan closest to the best on both ratios — walked to the *silver* end and burned as much stock as the dearest plan on the bar, so it saved nothing; the owner saw it, `docs/investigations/0020-the-plan-screen.md` §D-6.) |
| `PlanPick: 'more-mercs'` | **More mercs** | One step past the knee: the rung nearest the middle of the gap between the sweet spot and the top, strictly inside it — what the stock allows when it allows more than the knee and less than everything. Absent when the gap holds no rung. |
| `PlanPick: 'steady-max'` | **Steady max** | The top of the ladder — the most hired units the troops can shelter **every march of the horizon**, and the most damage a repeated march can do. (Named "Most mercs" until 2026-09-18, when **All in** stood past it: two stops both claiming "most" read as the same answer twice.) |
| `PlanPick: 'all-in'` | **All in** | The dear end of the bar, and the one stop that repeats no march: every mercenary the troops can shelter on the first march, then each next march on what the stock has left (`PlanTotals.sequence`). The campaign that spends the stock fastest. Its figures — the fold's row, the trade's columns, the bar's tip — are its **first** march's, so the fold's summary and the tip both say "*N* marches, each on what the last one left" instead of counting repeats (`sequenceWords`, `src/ui/sections/march/picks.ts`). |
| `PlanRow.bestFor` | **"best a silver" · "best a hired"** | Which of the two efficiencies a stop is the bar's best at. Exactly one stop of the bar is the best damage a silver and exactly one the best damage a hired unit; each says so in one muted line under its name, and in the row's accessible name and the bar's tip. They are notes, never rows (owner, 2026-09-17). |
...
| `recoveryPlan` | **Recovery plan** | Three whole rows, never a dropdown (rule 8): retrain everything · revive everything · **revive the top types, retrain the rest**. Under the third, **"Which families to revive"** — four checkboxes in two columns, guardsmen over monsters and specialists over engineers, each behind its own glyph and followed by **the tier it means** in the account's own shorthand, muted: "Top monsters M3", the same G/S/E/M letters the Troops card's head writes (owner, 2026-09-21: *"maybe we should also precise this next to top monsters / top guardsmen"*; a family the account does not field shows no tier). **Every stack at that tier** comes back from the Temple for gold — all four M3 monsters, not the best of them — and everything else is retrained. **No box for the mercenaries**: a hired unit cannot be recruited again, so the Temple returns it under every plan and there is nothing to decide (rule 15). A new setup opens on this plan with **the monsters alone** ticked — the slowest thing in the queue and what a player opens the Temple for after an epic march. The ticks are stored under every plan, so looking at another one and coming back does not lose them. |
| `marchFoot` (`#march-foot`) | **This march in full** | The March's explaining half, one panel: what the objective bought, the **Details** fold and the saved marches. The foot of the setup column on a desktop, the March sheet below 1200 px — never both (owner, 2026-09-15). |
| `MarchActions` | **the marks on the March's heading** | What a player does with a whole march — **Copy all counts · Edit counts · Undo · Save this march · Share** — as five 34 px marks beside the "12 stacks" meta, at both widths (owner, 2026-09-21: *"editing count, copy and share could be closer to summary and use icons to avoid crowding the ui"*). They were labelled buttons at the foot of the setup column, a page away from the figures and the pills they act on. The **name is the label** word for word, and the tooltip says it to a pointer; they are interface chrome, so Lucide and never emoji (rule 21). Only the edit toggle fills, and only while it is on — Generate is the page's one gilded control. **Esc** leaves the edit mode from inside a count field, where the hand already is. |
...
| `objectiveLocked` | **"The plan weighs damage against what it costs, so it decides this itself."** | What the Objective says for itself while **Complete optimization** is chosen, because that method picks the objective for itself. The Objective select is disabled and points at the sentence; on a phone the chip keeps its name and is named **"Objective: decided by the plan"** for a screen reader (owner, 2026-09-15 — a deliberate exception to §7.4's "hidden, not disabled"). **Where the sentence goes is the bar's room** (owner, 2026-09-21: *"could be on the right side of the objective to avoid too high bar… keep it on two lines"*): from 1200 px it is the select's own description, moved **beside** the well into a second column — two lines at 13 px in 16.25 rem, inside the height the label and the well already take, so the bar stays at its 87 px at every width from 1024 up. Below 1200 px, where the wells share the row with the answer and a phone's chips share 366 px, it is behind the **ⓘ** standing next to the control (`shell/ObjectiveWhy.tsx`), a popover and never a tooltip — a disabled control fires no hover and a thumb has none (rule 18). Printed under the well it cost the bar 33 px at 1400, 47 at 1100 and 44 at 390. |
