...
| `CampaignPlan.alternatives` | **The trade** | The plans nothing else beats on every resource at once, cut to the ones near the goal: the trade between silver, hired stock and damage. **The bar runs along the hired stock** (`CAMPAIGN.planBar.axis: 'burn'`, owner 2026-09-17 — the slider balances burning silver efficiently against burning mercenaries efficiently, and the sweet spot keeps the middle): one plan a burn level, **thriftiest first**, so a row's place on the bar is what a march of it burns for good. Its rows are the picks below — the plan's name is the row's own head, a `step` between two named answers is named by **what it burns** ("15 hired lost"), and the row carries a bar as long as the damage a march of it deals, measured against the loudest plan on the list. The **two efficiencies are notes on the stops that have them**, one muted line under the name in the trade's own words ("best a silver", "best a hired", `PlanRow.bestFor`), and never stops of their own: a separate "Best for silver" stop measured as the "Most damage" stop to 0.2 %, which the owner called inefficient and frustrating. The columns read one march at a time: **Plan** and then **Damage a march · Silver a march · Hired lost · Per silver · Per hired**, each head behind its glyph and the two seven-figure columns in compact figures (S-59) — and **Per silver and Per hired are the march's own ratios**, not the campaign's, so every figure on a row is a fact about the one march the recap above is drawing. A plan several of the definitions fit wears the first, in this order: **Sweet spot → Most damage → Best for silver → Spare the stock**. |
| `PlanPick: 'best-for-silver'` | **Best for silver** | The best damage a silver the band keeps, stated on the repeated march's own figures. A stop of the **silver** axis only, which stays selectable for comparison; on the app's own burn axis the same fact is the "best a silver" note on whichever stop holds it. |
...
| `PlanPick: 'sweet-spot'` | **Sweet spot** | **The middle of the trade in hired stock** — the best march whose burn is closest to the middle of the range between the thriftiest and the dearest plan the bar can carry. Where the bar opens, and its row says so in words. (Re-aimed 2026-09-16: the old rule — the plan closest to the best on both ratios — walked to the *silver* end and burned as much stock as the dearest plan on the bar, so it saved nothing; the owner saw it, `docs/investigations/0020-the-plan-screen.md` §D-6.) |
| `PlanPick: 'most-damage'` | **Most damage** | The most damage the army can do in a march. |
| `PlanPick: 'step'` | **"15 hired lost"** | A filler between two named answers on the burn axis. It answers no question of its own, so it is never called "Step": it is named by the hired units a march of it burns, in the same words as the column that figure is read off. |
