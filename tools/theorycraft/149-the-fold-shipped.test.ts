...
const planFor = (
  ...
  try {
    const plan = planCampaign({
      ...
      ...CAMPAIGN.planFixes,
      burnSaver: fold ? 'silver' : 'guard',
      foldTo: fold ? 5 : undefined,
...
describe.skipIf(!process.env.THEORY)('the fold, shipped', () => {
  it('runs the engine with the fold against the bar as shipped', () => {
    const profile = ownerProfile();
    const scenarios = [...commonScenarios(), ...(profile ? ownerScenarios(profile) : [])];
    const report = new Report('149-the-fold-shipped');
    report.add('# 149 — the fold, as the engine ships it\n');
    report.add(
      "**Today** is the bar as shipped in 54c7e3d (`burnSaver: 'guard'`); **fold** is the hired saver offered " +
        "everywhere and the bar re-chosen to at most five stops (`burnSaver: 'silver', foldTo: 5`). Figures are " +
